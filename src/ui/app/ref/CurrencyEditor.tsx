...
import { editorConfigCurrency, CurrencyListI, loadCurrencyList, newCurrencyList } from './currency'
import { GridColDef } from '@mui/x-data-grid'
import { Checkbox } from '@mui/material'

/*
  CRUD Editor for currencies
...
const CurrencyEditor = () => {
  ...
  //State 
  const [edConf, setEdConf] = useReducer(edConfRed, editorConfigCurrency()) 

  //Load list records
  const loadListCurrency = async() => {
    var list = await loadCurrencyList(setMessage, setSession)
    setEdConf ({type: ECF.list, payload : list})
  }

  //Create new entity
  const handleCreate = async () => {
    var l = await newCurrencyList(setMessage, setSession)
  ...
  //Set Changes
  const updateList = (entity : CurrencyListI, field : string, value : any) => {
    switch (field) {
      //add fields
    }
  ...
  const handleCheckboxClick = (id : number | undefined, field : string) => {
    if (id !== undefined) {
      var ent : CurrencyListI | null = getObjectById(Number(id), edConf.list)
      if (ent !== null) {
        type ObjectKey = keyof CurrencyListI;
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'descr', headerName: useLabel('currency'), width: 300, type: 'string', editable: editable, },
